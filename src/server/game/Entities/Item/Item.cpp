...
int32 Item::GenerateItemRandomPropertyId(uint32 item_id, Player const* player)
{
    ...
    {
        uint32 randomPropId = GetItemEnchantMod(itemProto->RandomProperty, player);
    ...
    {
        uint32 randomPropId = GetItemEnchantMod(itemProto->RandomSuffix, player);
...
Item* Item::CreateItem(uint32 item, uint32 count, Player const* player, bool clone, uint32 randomPropertyId, bool temp)
{
    ...
    if (!clone)
        pItem->SetItemRandomProperties(randomPropertyId ? randomPropertyId : Item::GenerateItemRandomPropertyId(item, player));
